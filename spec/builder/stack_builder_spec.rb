...
RSpec.describe 'StackBuilder' do
  ...
  describe '#build' do
    ...
    it 'should set the stack name, env, and user' do
      expect(Stack).to receive(:new).with('vpc', env: 'uat', user: 'tester')
      StackBuilder.new('vpc', env: 'uat', user: 'tester').build
    end

    it 'should set the template filename if supplied' do
      stack = instance_double('Stack')
      expect(Stack).to receive(:new).with('app1', env: 'dev', user: 'tester').and_return(stack)
      expect(stack).to receive(:template_filename=).with('app.json')
      StackBuilder.new('app1', env: 'dev', user: 'tester').template('app.json').build
    end
    ...
    it 'should set simple stack parameters' do
      ...
      StackBuilder.new('vpc', env: 'dev', user: 'tester').parameters(&params).build
    end

  ...
  describe '.load' do
    ...
    before(:each) do
      ...
      allow(cloudformation).to receive(:list_stacks).and_return(
        list_stacks_with(
          'murk-shared-vpc' => 'CREATE_COMPLETE',
          "murk-#{ENV['USER']}-webapp-network" => 'CREATE_COMPLETE'
        )
      )
      allow(cloudformation).to receive(:describe_stacks)
        .with(stack_name: 'murk-shared-vpc')
        .and_return(shared_vpc_describe_output)
      allow(cloudformation).to receive(:describe_stacks)
        .with(stack_name: "murk-#{ENV['USER']}-webapp-network")
